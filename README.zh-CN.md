...
![保存有效 Prompt，标出变化项，然后随用随填](assets/readme/reuse-flow.svg)

## Why｜为什么需要 Quiver？

有些 Prompt 太重要，不能每次凭记忆重写；但它们又太小、太偶发，没
必要成为 Skill、MCP 或常驻的 Agent 规则。

它可能是一句会改变答案质量的思维约束、一套反复打磨的讲解方式、一份
专业任务书，或一个已经验证有效的输出结构。Quiver 让这些文字保持精确、
随时可搜，却只在用户主动选择时进入当前对话。

它同时也是通用 Prompt 管理器：短句、完整 Prompt、可填写模板和长篇专业
指令，都放在同一个轻量库中。
...
如果 Quiver 为你节省了时间，可以自愿支持后续开发。以下暂时使用个人收款码，
仅用于自愿赞赏，不作为购买商品或服务的付款入口；确认付款前，请务必核对支付
App 显示的收款方和金额。

| 支付宝 | 微信支付 |
|:---:|:---:|
| <img src="assets/support/alipay-qr.JPG" width="220" alt="支付宝自愿赞赏收款码"> | <img src="assets/support/wechat-pay-qr.JPG" width="220" alt="微信支付自愿赞赏收款码"> |

Quiver 不会在软件内投放广告、跟踪 Prompt 内容或出售用户数据访问权。合作联系
方式和国际支持入口完成核验后，也会直接添加在这里。
