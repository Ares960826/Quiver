...
还在逐个文件翻找，一遍遍复制粘贴——小拇指都快变成专职快捷键了？

好不容易找到的 Prompt，用完一次，下次又不见了；花一个小时精雕细琢，遇到
同类任务还得从头重写。真正的工作五分钟，Prompt 两小时。

Quiver 给值得保留的文字一个固定发射台：保存一次，变成 Arrow；把每次变化的
关键词挖成空格子；下次只填写新的部分——搜索、填写、发射。

有些 Prompt 太重要，不能每次凭记忆重写；但它们又太小、太偶发，不值得专门
做成 Skill、MCP 或常驻的 Agent 规则。
