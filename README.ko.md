...
아직도 아껴 둔 프롬프트를 Notion, Typora, Obsidian 곳곳에 흩어 놓고 있나요?
여기 한 부, 저기 더 최신인 한 부, 어딘가에는 `진짜-최종-최종본`까지? 보관은
완벽합니다. 정작 필요할 때 찾을 수 없다는 점만 빼면요.

아직도 파일을 뒤지고 한 줄씩 선택해 복사하고 붙여 넣나요? `⌘C`와 `⌘V`는 이미
프로 선수가 되었는데 실제 작업은 시작도 못 했습니다. 5분짜리 작업, 2시간짜리
프롬프트 준비—AI는 야근하지 않았는데 이상하게 사용자는 야근 중입니다.

한 시간 동안 다듬은 프롬프트가 정말 한 번 멋지게 일하고 바로 은퇴해야 할까요?
오늘은 주제, 내일은 대상이나 언어만 바뀝니다. 가치 있는 추론 골격, 말투, 출력
구조는 그대로인데 단어 세 개가 달라졌다고 ‘두뇌’까지 다시 써야 할 이유가 있나요?

그래서 Quiver가 존재합니다. 효과가 검증된 프롬프트를 **Arrow**로 저장하고,
매번 달라지는 부분만 슬롯으로 만든 다음, 다음에는 새로운 값만 채우세요.
검색하고, 채우고, 발사하면 됩니다.

Skill이나 MCP로 만들기에는 너무 작나요? 오히려 딱 맞습니다. 슬롯이 전혀 없나요?
그것도 좋습니다. 짧은 문구, 완성된 프롬프트, 입력형 템플릿, 긴 전문 지시문을
정확하고 검색 가능한 상태로 보관하되 사용자가 선택한 순간에만 대화에 넣습니다.
모든 대화에 상주하며 조용히 컨텍스트를 편향시키지도 않습니다.
