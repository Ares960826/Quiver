...
![효과가 검증된 프롬프트를 저장하고, 바뀌는 부분을 표시한 뒤 어디서든 다시 사용하세요](assets/readme/reuse-flow.svg)

## Why｜왜 Quiver인가요?

어떤 프롬프트는 기억에 의존해 다시 쓰기에는 너무 중요하지만, Skill, MCP,
상시 에이전트 규칙으로 만들기에는 너무 작고 상황 의존적입니다.

답변을 바꾸는 사고 제약, 정성껏 다듬은 설명 방식, 전문 브리프, 이미 효과가
검증된 출력 구조가 여기에 해당합니다. Quiver는 그 문구를 정확하고 검색 가능한
상태로 보관하며, 사용자가 선택한 순간에만 현재 대화에 넣습니다.

짧은 문구, 완성된 프롬프트, 입력 가능한 템플릿, 긴 전문 지시문을 모두 같은
가벼운 라이브러리에서 관리하는 범용 프롬프트 관리자이기도 합니다.
...
아래 개인 수금 코드는 임시 팁 용도이며 상품이나 서비스 결제용이 아닙니다.
결제 전에 결제 앱에 표시된 수취인과 금액을 반드시 확인하세요.

| Alipay | WeChat Pay |
|:---:|:---:|
| <img src="assets/support/alipay-qr.JPG" width="220" alt="Alipay 자발적 후원 QR 코드"> | <img src="assets/support/wechat-pay-qr.JPG" width="220" alt="WeChat Pay 자발적 후원 QR 코드"> |

Quiver는 앱 안에 광고를 넣거나 프롬프트 내용을 추적하거나 사용자 데이터 접근
권한을 판매하지 않습니다. 파트너십 연락처와 국제 후원 수단은 확인 후 이곳에
추가됩니다.
