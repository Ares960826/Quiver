...
파일을 다시 뒤져 한 줄씩 복사하고 붙여 넣느라 새끼손가락을 단축키 전담으로
쓰고 있지는 않나요?

한 시간 동안 다듬은 프롬프트가 한 번은 훌륭하게 작동하지만, 비슷한 다음 작업이
오면 또 사라져 있습니다. 실제 작업은 5분, 프롬프트를 다시 쓰는 데는 2시간입니다.

Quiver는 보존할 가치가 있는 문구에 고정된 발사대를 제공합니다. 한 번 Arrow로
저장하고, 매번 바뀌는 단어만 슬롯으로 만든 다음, 다음에는 새로운 값만 채우세요.
검색하고, 채우고, 발사하면 됩니다.
